...
def compute_metrics(pred, evaluating):
    ...
    if evaluating:
        return {
            ...
            'probabilities': scipy.special.softmax(logits, axis=1).tolist()
        }
    else:
        return {
            'accuracy': acc,
...
def load_data_as_features(data, tokenizer):
    ...
    for (s1, s2, el, rl) in data:
        di = tokenizer(s1.lower(), s2.lower(), max_length=512, padding='max_length')
...
class BERTFineTuner(object):
    def __init__(self, name, tokenizer, model, train_dataset, dev_dataset, dataset_name, evaluating,
    ...
    def setup_train_args(self):
        batch_size = 8
        outdir_name = f'./results_{self.model_name}'
        logdir_name = f'./logs_{self.model_name}'
    ...
    def setup_trainer(self):
        return Trainer(
            ...
            compute_metrics=lambda x: compute_metrics(x, self.evaluating)
        )
